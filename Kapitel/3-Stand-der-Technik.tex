...
Ein Beispiel hierfür sind die sogenannten \s{FML} (englisch für Fibre Metal Laminates, deutsche Übersetzung: Faser-Metall-Verbunde)~.
...
Die erstmals im großem Maßstab zum Einsatz kommenden FML bestanden aus Aluminiumblechen und Glasfaser- oder Aramidfaserlaminaten, (Bezeichnung: Glare bzw.\ \s{Arall})~\cite{Vogelesang2000}.
...
Magnesium bietet verschiedene Vorteile, unter anderem eine geringe Dichte und gute Abschirmfähigkeiten gegenüber elektromagnetischer Strahlung, die im Flugzeugbau eine wichtige Eigenschaft darstellt~\cite{Cortes2005}.
...
Bellini et al.~\cite{Bellini2020} untersuchte die Einflüsse von Aufbau und Grenzflächendesign auf die Biegefestigkeit von \s{Carall}, also eines Verbundes aus Aluminiumblech und kohlefaserverstärkten Kunststoffen.
...
Kuhtz et al.~\cite{Kuhtz2019} untersuchten die Versagenseigenschaften eines mehrlagigen FML aus Magnesium- oder Aluminiumblech und kohlefaserverstärkter \s{PA}6-Matrix.
...
Klassischerweise wurde in den ersten FML wie \s{Glare} ein Epoxidharz genutzt, der durch eine chemische Reaktion bei einer erhöhten Temperatur aushärtet.
...
Behrens et al.~\cite{Behrens2019} nutzt zusätzlich zum normalen Matrixwerkstoff PA6 mit Kohlefaserverstärkung an den Grenzflächen zu Metallschichten je einen $0,2 \, mm$ dicken Film PA6 ohne Faserverstärkung.
...
Eine andere Definition beschreibt die Haftung $\sigma_H$ als innere Kraft \s{$F_i$} auf die wahre Oberfläche \s{$A_w$} bezogen, was die jeweilige Rauheit der Oberfläche mit einschließt \autoref{eq:Haftung}~\cite{Bischof1993}.
...
Die Adhäsion charakterisiert eine Kraft \s{F}, die in den jeweiligen Oberflächen von zwei Fügepartnern wirkt und dabei vom Abstand der Oberflächen abhängt.
...
Dabei sind die Molekülsegmente unterhalb der Glasübergangstemperatur \s{$T_g$} nicht ausreichend beweglich~\cite{Seidler1971}~.
...
Die Haftung ist das Resultat chemischer Reaktionen zwischen Metall und Polymer~\cite{Haefer1987}.
...
Zhou et al.~\cite{Zhou2021} stellt eine Möglichkeit vor, mit der FML auf Magnesiumbasis mithilfe einer Zwischenschicht aus einer eutektischen \s{Mg}-\s{Zn}-\s{Al}-Legierung haltbar verbunden werden.
...
Zusätzlich wurden die Kohlefasern mit einer \s{Ni}-Schicht beschichtet, um eine bessere Verteilung des Epoxidharzes zu erreichen.
...
Eine bisher wenig betrachtete Möglichkeit ist die Vorbehandlung des Matrixwerkstoffes, in diesem Fall Polypropylen (\s{PP}), mit einer Plasmafunktionalisierung.
...
\begin{figure}[H]%[htbp]
    \centering
    \includegraphics[width=0.6\linewidth]{Bilder/SdT/Laserstrukturiert}
...
Die Prägewerkzeuge wurden mit eine Flächenpressung von $125\, MPa$ auf die Werkstücke gedrückt und somit die Oberflächenstrukturen in Bleche der Magnesiumlegierung AZ31 und Aluminiumlegierung \s{AW} 5754 eingebracht.
...
Dabei können auch Profile aus mehreren Lagen hergestellt werden, sowohl mit Prepreg-Verfahren (Epoxidharzmatrix, welche im Anschluss ausgehärtet wird) als auch mit thermoplastischer Matrix~\cite{Sinke2007}.
...
Behrens et al. \cite{Behrens2017} untersuchten die Umformung eines faserverstärkten Kunststoffes, aber mit normalem Tiefziehwerkzeug.
...
Die Untersuchung von Behrens et al.~\cite{Behrens2019} setzt das Ziel, die Prozesszeiten einer Massenfertigung unter \SI{20}{\second} zu bringen.
